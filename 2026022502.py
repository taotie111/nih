"""
================================================================================
2026022502 - 排除青光眼 + PSM(age+sex) + Cox调age+sex(不调BMI)
...
def main():
    print("="*70)
    print("2026022502 - 排除青光眼 + PSM(age+sex) + Cox调age+sex(不调BMI)")
    print("="*70)
    ...
    covars_model3 = ['age_baseline', 'sex']  # 仅调age+sex，不调BMI
    ...
    model_results = []
    print(f"\n{'':12s} | {'Model1(age+sex)':^20s} | {'Model2(+BMI+lifestyle)':^20s} | {'Model3(age+sex)':^20s}")
    print("-"*80)
    ...
    for name, col in [('DHA', 'fatty_acids_dha'), ('Omega-3', 'fatty_acids_n3')]:
        print(f"\n--- {name} ---")
        q_res, trend_p = quartile_analysis(df_matched, col, covars_model3)
        for r in q_res:
            s = "*" if r['P'] < 0.05 else ""
            print(f"  {r['quartile']}: HR={r['HR']:.2f} ({r['CI']}), P={r['P']:.4f} {s}")
        print(f"  P for trend: {trend_p:.4f}")
        quartile_results.append({'Variable': name, 'Trend_P': trend_p})
    ...
    for sex_val in [0, 1]:
        ...
        if len(df_sex) > 100:
            for name, col in [('DHA', 'fatty_acids_dha'), ('Omega-3', 'fatty_acids_n3')]:
                r = cox_regression(df_sex, col, covars_model3)
                s = "*" if r['P'] < 0.05 else ""
                print(f"  {label}-{name}: HR={r['HR']:.2f} ({r['CI_low']:.2f}-{r['CI_high']:.2f}), P={r['P']:.4f} {s}")
                stratified_results.append({'Stratum': f'Sex={label}', 'Variable': name, 'HR': r['HR'], 'CI': f"{r['CI_low']:.2f}-{r['CI_high']:.2f}", 'P': r['P']})
    ...
    for ag in ['<55', '55-64', '>=65']:
        ...
        if len(df_age) > 100:
            for name, col in [('DHA', 'fatty_acids_dha'), ('Omega-3', 'fatty_acids_n3')]:
                r = cox_regression(df_age, col, covars_model3)
                s = "*" if r['P'] < 0.05 else ""
                print(f"  Age {ag}-{name}: HR={r['HR']:.2f} ({r['CI_low']:.2f}-{r['CI_high']:.2f}), P={r['P']:.4f} {s}")
                stratified_results.append({'Stratum': f'Age={ag}', 'Variable': name, 'HR': r['HR'], 'CI': f"{r['CI_low']:.2f}-{r['CI_high']:.2f}", 'P': r['P']})
    ...
    for bmi in ['<25', '25-30', '>=30']:
        ...
        if len(df_b) > 100:
            for name, col in [('DHA', 'fatty_acids_dha'), ('Omega-3', 'fatty_acids_n3')]:
                r = cox_regression(df_b, col, covars_model3)
                s = "*" if r['P'] < 0.05 else ""
                print(f"  BMI {bmi}-{name}: HR={r['HR']:.2f} ({r['CI_low']:.2f}-{r['CI_high']:.2f}), P={r['P']:.4f} {s}")
                stratified_results.append({'Stratum': f'BMI={bmi}', 'Variable': name, 'HR': r['HR'], 'CI': f"{r['CI_low']:.2f}-{r['CI_high']:.2f}", 'P': r['P']})
    ...
    for name, col in [('DHA', 'fatty_acids_dha'), ('Omega-3', 'fatty_acids_n3')]:
        ...
        exog_vars = [col, 'sex', 'interaction'] + covars_model3
        ...
        try:
            ...
            interaction_results.append({'Variable': name, 'Interaction': 'sex', 'P': p_int})
        except Exception as e:
            print(f"  {name} x sex: Error - {e}")
    ...
    if len(df_noamd) > 1000:
        ...
        for name, col in [('DHA', 'fatty_acids_dha'), ('Omega-3', 'fatty_acids_n3')]:
            r = cox_regression(df_m_noamd, col, covars_model3)
            s = "*" if r['P'] < 0.05 else ""
            print(f"  {name}: HR={r['HR']:.2f} ({r['CI_low']:.2f}-{r['CI_high']:.2f}), P={r['P']:.4f} {s}")
            sensitivity_results.append({'Analysis': 'Exclude_AMD', 'Variable': name, 'HR': r['HR'], 'CI': f"{r['CI_low']:.2f}-{r['CI_high']:.2f}", 'P': r['P'], 'N': r['N']})
    ...
    if len(df_2yr) > 1000:
        ...
        for name, col in [('DHA', 'fatty_acids_dha'), ('Omega-3', 'fatty_acids_n3')]:
            r = cox_regression(df_m_2yr, col, covars_model3)
            s = "*" if r['P'] < 0.05 else ""
            print(f"  {name}: HR={r['HR']:.2f} ({r['CI_low']:.2f}-{r['CI_high']:.2f}), P={r['P']:.4f} {s}")
            sensitivity_results.append({'Analysis': 'Exclude_FU<2yr', 'Variable': name, 'HR': r['HR'], 'CI': f"{r['CI_low']:.2f}-{r['CI_high']:.2f}", 'P': r['P'], 'N': r['N']})
    
    # 排除糖尿病眼病
    print("\n--- 排除糖尿病眼病 ---")
    df_nodr = df[df.get('diabetic_retinopathy_baseline', 0) == 0]
    if len(df_nodr) > 1000:
        df_m_nodr = psm_match(df_nodr, match_vars=['age_baseline', 'sex'])
        for name, col in [('DHA', 'fatty_acids_dha'), ('Omega-3', 'fatty_acids_n3')]:
            r = cox_regression(df_m_nodr, col, covars_model3)
            s = "*" if r['P'] < 0.05 else ""
            print(f"  {name}: HR={r['HR']:.2f} ({r['CI_low']:.2f}-{r['CI_high']:.2f}), P={r['P']:.4f} {s}")
            sensitivity_results.append({'Analysis': 'Exclude_DiabeticRetinopathy', 'Variable': name, 'HR': r['HR'], 'CI': f"{r['CI_low']:.2f}-{r['CI_high']:.2f}", 'P': r['P'], 'N': r['N']})
    ...
    df_matched.to_csv('2026022502_matched_data.csv', index=False)
    print(f"匹配数据: 2026022502_matched_data.csv (N={len(df_matched)})")
    
    pd.DataFrame(model_results).to_excel('2026022502_Table2_MultiModel_Cox.xlsx', index=False)
    print("Table2 (多模型Cox): 2026022502_Table2_MultiModel_Cox.xlsx")
    
    if stratified_results:
        pd.DataFrame(stratified_results).to_excel('2026022502_Table3_Stratified.xlsx', index=False)
        print("Table3 (分层分析): 2026022502_Table3_Stratified.xlsx")
    
    if interaction_results:
        pd.DataFrame(interaction_results).to_excel('2026022502_Table4_Interaction.xlsx', index=False)
        print("Table4 (交互检验): 2026022502_Table4_Interaction.xlsx")
    
    if sensitivity_results:
        pd.DataFrame(sensitivity_results).to_excel('2026022502_Table5_Sensitivity.xlsx', index=False)
        print("Table5 (敏感性分析): 2026022502_Table5_Sensitivity.xlsx")
